  ...
  apply(compiler) {
    compiler.plugin.bind(
      ...
      for (let x in entrypoints) {
        ...
        let text = template
          .replace("[css]", insert_css)
          .replace("[js]", insert_js);
        console.log('HTML:', path.join(compilation.options.output.path, x + ".html"))
        fs.writeFileSync(
          path.join(compilation.options.output.path, x + ".html"),
          text
        );
      }
    });
